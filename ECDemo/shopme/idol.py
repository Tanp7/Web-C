...
# 创建蓝图对象
idol = Blueprint('idol',__name__)

# idol列表
@idol.route('/',methods=['GET','POST'])
def index():
	...
	# 因此需要 tproduct 和 tidol 两张表联合查询
	sql = 'select count(*) from tidol t1,tproduct t2 where t1.proid=t2.id'
	...
	if total>0:
		...
		sql = 'select t1.id,t1.proid,t1.seqno,t2.code,t2.name,t2.priceold,t2.pricenew,t2.priceunit from tidol t1,tproduct t2 where t1.proid=t2.id'
		if type!='':
			sql += ' and (t2.type1 like :type or t2.type2 like :type or t2.tags like :type)'
			dt_conn['type']='%'+type+'%'
		if keyword!='':
	...
	return render_template('admin_idol.html',login_admin='',data_modal=data_modal)
		
# 删除记录
@idol.route('/delete.do',methods=['POST'])
def delete():
	id = int(request.form['id'])
	conn = sqlite3.connect(pju.get_db_path())
	cursor = conn.cursor()
	sql = 'delete from tidol where id=?'
...
# 调整排序号
@idol.route('/setseqno.do',methods=['POST'])
def setseqno():
	id = int(request.form['id'])
	seqno = int(request.form['seqno'])
	conn = sqlite3.connect(pju.get_db_path())
	cursor = conn.cursor()
	sql = 'update tidol set seqno=? where id=?'
...
@idol.route('/setprice.do',methods=['POST'])
def setprice():
